...
public class ImageListAdapter extends BaseAdapter {
	...
	// Constructor
	public ImageListAdapter(Context c,int pixelHeight) {

		myContext = c;
		thumbHeight=pixelHeight;
		thumbWidth=pixelHeight;
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if(cache[position] == null) {
			...
			//myContext.
			//myContext.	getWindowManager().getDefaultDisplay().getMetrics(metrics);
			//int pixelHeight=metrics.heightPixels;
			//int pixelWidth=metrics.widthPixels;
			BitmapFactory.Options options = new BitmapFactory.Options();
			
			
			Bitmap scaled=BitmapFactory.decodeResource(myContext.getResources(), images[position], options);
			int height=scaled.getHeight();
			int width=scaled.getWidth();
			if(width>height)
			scaled=Bitmap.createScaledBitmap(scaled, height, height, true);
			else scaled =Bitmap.createScaledBitmap(scaled, width, width, true);
			
			//options.inSampleSize = 4;
			//options.outWidth=thumbWidth;
			//options.outHeight=thumbHeight;
			
			
			Bitmap thumb = Bitmap.createScaledBitmap(scaled,(scaled.getHeight()/8),(scaled.getHeight()/8), false);//BitmapFactory.decodeResource(myContext.getResources(), images[position], options);
			//BitmapFactory.
